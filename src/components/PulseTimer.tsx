...
export function PulseTimer({ time = '03:00', pulsing = false, warning = false }: PulseTimerProps) {
  return (
    <div className="relative flex items-center justify-center py-[2.75rem] my-[2.75rem]">
      ...
      <div
        className={`z-10 font-display text-[6rem] md:text-[8rem] tabular-nums tracking-tighter leading-none drop-shadow-lg transition-colors duration-500 line-clamp-1 ${warning ? 'text-brand-error' : 'text-brand-on-surface'}`}
      >
        {time}
      </div>
    </div>
