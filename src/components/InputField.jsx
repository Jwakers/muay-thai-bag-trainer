...
export function InputField({ label, error, className = '', ...props }) {
  return (
    <div className={`flex flex-col mb-[1.4rem] ${className}`}>
      {label && <label className="font-label text-[0.75rem] text-brand-outline mb-2 uppercase tracking-wide">{label}</label>}
      <input 
        ...
        {...props}
      />
      {error && <span className="font-label text-[0.75rem] text-brand-error mt-2">{error}</span>}
    </div>
  );
}
